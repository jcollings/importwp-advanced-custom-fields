...
git clone --branch $BRANCH git@github.com:jcollings/importwp-advanced-custom-fields.git $FOLDER
cd $FOLDER
git rm -rf .
...
sed -i -e "s/__STABLE_TAG__/$TAG/g" advanced-custom-fields.php

# Confirm pushing of build.
while true; do

read -p "Do you want to push build ImportWP ACF v$TAG? (y/n) " yn
